# Global dependencies
from datasets.oai_config import (
    IMG_DIR_PATH,
    CLINICAL_CONTROL_COLUMNS,
    IMG_CODES_FILENAME,
    NON_IMG_DATA_FILENAME,
...
class PytorchImagesDataset(Dataset):
    ...
    def __init__(
        ...
        assert (
            len(set(y_cols).intersection(set(C_cols))) == 0
        )  # A and y should not share variables

        assert transform in ["None", "random_translation"], "transform is: %s" % str(
            transform
        )
        if transform != "None":
            assert max_horizontal_translation > 0
        if transform != "None":
            assert max_vertical_translation > 0
        ...
        # ----- Data processing -----
        (
            self.base_dir_for_images,
            self.non_image_data,
            data_transform_statistics,
        ) = load_non_image_data(
            self.dataset,
            C_cols,
            y_cols,
        ...
        if len(C_cols) > 0:
            ...
            self.C_feats_loss_class_wts = copy.deepcopy(
                self.non_image_data[variables].values
            )
    ...
    def __getitem__(self, idx):
        ...
        if not self.cache or not cache_hit:
            image_path = os.path.join(
                self.base_dir_for_images, "image_%i.npy" % new_idx
            )
            image = self.load_image(image_path)

        # ----- Data augmentation -----
        if self.transform == "random_translation":
            image = random_transform_image(
                image,
                self.transform,
        ...
        if self.C_cols:
            ...
            C_feats_not_nan = C_feats_not_nan * 1.0
        ...
        y_feats = torch.as_tensor(y_feats).squeeze().long()
        C_feats = torch.as_tensor(C_feats).float()
        C_feats_not_nan = torch.as_tensor(C_feats_not_nan).float()
        C_feats_loss_class_wts = torch.as_tensor(C_feats_loss_class_wts).float()
        concepts = (C_feats, C_feats_not_nan, C_feats_loss_class_wts)

        return (image, concepts), y_feats
...
def load_non_image_data(
    ...
    image_codes = pickle.load(
        open(os.path.join(base_dir_for_images, IMG_CODES_FILENAME), "rb")
    )
    non_image_data = pd.read_csv(
        os.path.join(base_dir_for_images, NON_IMG_DATA_FILENAME), index_col=0
    )
    if check:
        ensure_barcodes_match(non_image_data, image_codes)
    ...
    if truncate_C_floats:
        ...
        for variable in C_cols + y_cols:
            ...
            non_image_data[variable] = (
                non_image_data[variable].values.astype(np.int64).astype(np.float64)
            )
    ...
    if zscore_Y:
        ...
        for i in range(len(y_cols)):
            ...
            if verbose:
                print(
                    "Z-scoring additional feature %s with mean %2.3f and std %2.3f"
                    % (y_cols[i], mu, std)
                )
...
def get_sampling_weights(
    sampling_strategy, sampling_args, train_dataset, C_cols, y_cols
):
    """
    ...
    if sampling_strategy == "weighted":
        ...
        if sampling_args["mode"] in ["weigh_C", "weigh_Cy"]:
            ...
            for i in range(N_C_cols):
                ...
                total_wt_per_class = 1.0 / N_classes
                ...
                for cls_val in classes:
                    ...
                    if counts < sampling_args["min_count"]:
                        ...
                        print(
                            "Setting Attribute %s (Class %f) to uniform due to small size of %d"
            ...
            print(
                "Histogram weights              : ",
                " ".join(["%.1f" % x for x in hist_weights]),
            )
            ...
            print(
                "Uniform prob mass (%)          :",
                " ".join(["%.1f" % (x * 100) for x in unif_prob_mass_per_bin]),
            )
            prob_mass = hist_weights[:-1] * hist_counts
            prob_mass_per_bin = prob_mass / np.sum(prob_mass)
            print(
                "Histogram weights prob mass (%): ",
                " ".join(["%.1f" % (x * 100) for x in prob_mass_per_bin]),
                "\n",
            )
...
def load_data_from_different_splits(
    batch_size,
    C_cols,
    y_cols,
    zscore_C,
    ...
    train_sampler, shuffle = get_sampling_weights(
        sampling_strategy, sampling_args, train_dataset, C_cols, y_cols
    )
    ...
    test_dataset = PytorchImagesDataset(
        dataset="test",
        transform_statistics=train_dataset.transform_statistics,
        C_cols=C_cols,
        y_cols=y_cols,
...
def ensure_barcodes_match(combined_df, image_codes):
    ...
    for idx in range(len(combined_df)):
        ...
        if image_codes[idx] != code_in_df:
            raise Exception(
                "Barcode mismatch at index %i, %s != %s"
                % (idx, image_codes[idx], code_in_df)
            )
    print("All %i barcodes line up." % len(combined_df))


# ----------------- Data augmentations -----------------
def random_horizontal_vertical_translation(
    img, max_horizontal_translation, max_vertical_translation
):
    """
    ...
    horizontal_translation = int(
        (random.random() - 0.5) * max_horizontal_translation * width
    )
    vertical_translation = int(
        (random.random() - 0.5) * max_vertical_translation * height
    )
    ...
    if horizontal_translation != 0:
        if horizontal_translation > 0:
            ...
            ]  # cuts off pixels on the left of image.
        else:
            translated_img = translated_img[
                :, :, :horizontal_translation
            ]  # cuts off pixels on the right of image.

    if vertical_translation != 0:
        if vertical_translation > 0:
            translated_img = translated_img[
                :, vertical_translation:, :
            ]  # cuts off pixels on the top of image.
        else:
            translated_img = translated_img[
                :, :vertical_translation, :
            ]  # cuts off pixels on the bottom of image.
...
def random_transform_image(
    image, transform, max_horizontal_translation, max_vertical_translation
):
    assert transform in [
        "random_translation_and_then_random_horizontal_flip",
        "random_translation",
    ]
    image = random_horizontal_vertical_translation(
        image, max_horizontal_translation, max_vertical_translation
    )
...
def downsample_image(image, downsample_fraction):
    ...
    for i in range(3):  # RGB
        ...
        img2 = cv2.resize(
            img,
            (
                int(original_size[1] * downsample_fraction),
                int(original_size[0] * downsample_fraction),
            ),
        )
