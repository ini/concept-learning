...
from datasets.pitfalls import DatasetC
...
train_loader = DataLoader(
    DatasetC(num_concepts=concept_dim, train=True), batch_size=64, shuffle=True)
test_loader = DataLoader(
    DatasetC(num_concepts=concept_dim, train=False), batch_size=64, shuffle=False)



def sequential_training():
    model = ConceptBottleneckModel(input_dim=28*28, output_dim=2, concept_dim=concept_dim)

    # Train concept predictor
    concept_predictor = nn.Sequential(get_base_model(28*28, concept_dim), nn.Sigmoid())
    ...
    target_predictor = get_base_model(concept_dim, 2)
    train_multiclass_classification(
        target_predictor,
        train_loader,
