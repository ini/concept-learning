...
def load_train_results(
    ...
    groupby: list[str] = ["dataset", "model_type"],
) -> Iterable[Result]:
    """
...
def load_model(
    result: Result, loader: DataLoader | None = None
...
def evaluate(config: dict):
    ...
    if config["eval_mode"] == "accuracy":
        ...
        if "test_concept_acc" in trainer.callback_metrics:
            metrics["test_concept_acc"] = trainer.callback_metrics[
                "test_concept_acc"
            ].item()
        else:
            metrics["test_concept_acc"] = 0.0

        if "concept_loss" in trainer.callback_metrics:
            metrics["test_concept_loss"] = trainer.callback_metrics[
                "concept_loss"
            ].item()
        else:
            metrics["test_concept_loss"] = 0.0
