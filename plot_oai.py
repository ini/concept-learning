from plot import *
from lightning_ray import filter_by
...
if __name__ == '__main__':
    ...
    settings = [("latent_residual", None), ("decorrelated_residual", None), ("mi_residual", None), ("latent_residual", "iter_norm")]
    for name, setting in zip(names, settings):
        ...
        for dataset_name, dataset_results in results.items():
            for mode in args.mode:
                ...
                def fun(config):
                    return config['model_type'] == setting[0] and config['norm_type'] == setting[1]
                filtered_results = filter_by(dataset_results,fun)
                PLOT_FUNCTIONS[mode](
                    filtered_results,
                    dataset_name,
